...
target_idx_list = 3:5;          % Evaluate 3 targets and average their RMSE.
...
snr_db_list = -20:5:30;
snr_db_fixed = 10;
antenna_list = [8,16,33,64,128,256];
subcarrier_list = [16, 32, 64, 128, 256];
...
if run_snr_sweep
    ...
    for is = 1:numel(snr_db_list)
        ...
        rmse_vs_snr(:, is) = run_mc_block(p_cur, truth, algorithms, target_idx_list, snr_db, mc_trials, use_parallel, seed_base + 10000 * is);
...
if run_antenna_sweep
    ...
    for ia = 1:numel(antenna_list)
        ...
        rmse_vs_ant(:, ia) = run_mc_block(p_cur, truth, algorithms, target_idx_list, snr_db_fixed, mc_trials, use_parallel, seed_base + 20000 * ia);
...
if run_subcarrier_sweep
    ...
    for in = 1:numel(subcarrier_list)
        ...
        p_cur = configure_dft_grid_and_peak_params(p_cur);
        rmse_vs_ns(:, in) = run_mc_block(p_cur, truth, algorithms, target_idx_list, snr_db_fixed, mc_trials, use_parallel, seed_base + 30000 * in);
...
if run_snr_sweep
    fig1 = plot_error_curve(snr_db_list, rmse_vs_snr, algorithms, style, ...
        'SNR (dB)', sprintf('Average Angle RMSE over targets %d-%d (deg)', target_idx_list(1), target_idx_list(end)), ...
        sprintf('Multi-Target 1-bit Angle RMSE vs SNR (Targets %d-%d, averaged)', target_idx_list(1), target_idx_list(end)));
end

if run_antenna_sweep
    fig2 = plot_error_curve(antenna_list, rmse_vs_ant, algorithms, style, ...
        'Receive Antenna Number M_{rx}', sprintf('Average Angle RMSE over targets %d-%d (deg)', target_idx_list(1), target_idx_list(end)), ...
        sprintf('Multi-Target 1-bit Angle RMSE vs Receive Antenna Number (SNR = %+d dB, averaged over targets)', snr_db_fixed));
else
    fig2 = [];
end
...
if run_subcarrier_sweep
    fig3 = plot_error_curve(subcarrier_list, rmse_vs_ns, algorithms, style, ...
        'Subcarrier Number N_s', sprintf('Average Angle RMSE over targets %d-%d (deg)', target_idx_list(1), target_idx_list(end)), ...
        sprintf('Multi-Target 1-bit Angle RMSE vs Subcarrier Number (SNR = %+d dB, averaged over targets)', snr_db_fixed));
else
    fig3 = [];
end

out_png_1 = fullfile(pwd, sprintf('rmse_vs_snr_targets_%d_%d_%d_1bit_compare.png', target_idx_list(1), target_idx_list(2), target_idx_list(3)));
out_pdf_1 = fullfile(pwd, sprintf('rmse_vs_snr_targets_%d_%d_%d_1bit_compare.pdf', target_idx_list(1), target_idx_list(2), target_idx_list(3)));
out_png_2 = fullfile(pwd, sprintf('rmse_vs_antenna_targets_%d_%d_%d_1bit_compare.png', target_idx_list(1), target_idx_list(2), target_idx_list(3)));
out_pdf_2 = fullfile(pwd, sprintf('rmse_vs_antenna_targets_%d_%d_%d_1bit_compare.pdf', target_idx_list(1), target_idx_list(2), target_idx_list(3)));
out_png_3 = fullfile(pwd, sprintf('rmse_vs_subcarrier_targets_%d_%d_%d_1bit_compare.png', target_idx_list(1), target_idx_list(2), target_idx_list(3)));
out_pdf_3 = fullfile(pwd, sprintf('rmse_vs_subcarrier_targets_%d_%d_%d_1bit_compare.pdf', target_idx_list(1), target_idx_list(2), target_idx_list(3)));
out_png_4 = [];
out_pdf_4 = [];
out_mat = fullfile(pwd, sprintf('multitarget_threefig_targets_%d_%d_%d_1bit_compare.mat', target_idx_list(1), target_idx_list(2), target_idx_list(3)));
...
if run_antenna_sweep && ~isempty(fig2)
    exportgraphics(fig2, out_png_2, 'Resolution', 300);
    exportgraphics(fig2, out_pdf_2, 'ContentType', 'vector');
end

if run_subcarrier_sweep && ~isempty(fig3)
    exportgraphics(fig3, out_png_3, 'Resolution', 300);
    exportgraphics(fig3, out_pdf_3, 'ContentType', 'vector');
end

% No spectrum comparison figure in this averaged RMSE script.

save(out_mat, ...
    'p0', 'truth', 'algorithms', 'target_idx_list', 'mc_trials', 'desired_workers', ...
...
%% ===== Local functions =====
function rmse_vec = run_mc_block(p, truth, algorithms, target_idx_list, snr_db, mc_trials, use_parallel, seed_offset)
num_alg = numel(algorithms);
...
if use_parallel
    parfor it = 1:mc_trials
        err_sq(:, it) = run_one_trial(p, truth, algorithms, target_idx_list, snr_db, seed_offset + it);
    end
else
    for it = 1:mc_trials
        err_sq(:, it) = run_one_trial(p, truth, algorithms, target_idx_list, snr_db, seed_offset + it);
...
function err_sq_vec = run_one_trial(p, truth, algorithms, target_idx_list, snr_db, trial_seed)
rng(trial_seed, 'twister');
[y, x] = gen_multi_target_frame(p, truth, snr_db);

num_alg = numel(algorithms);
num_targets_eval = numel(target_idx_list);
err_sq_targets = zeros(num_alg, num_targets_eval);
...
for ia = 1:num_alg
    ...
    for it = 1:num_targets_eval
        target_idx = target_idx_list(it);
        theta_true = truth.theta_deg(target_idx);
        theta_hat = select_estimate_for_target(theta_hat_all, theta_true);
        err_sq_targets(ia, it) = wrap_to_180(theta_hat - theta_true)^2;
    end
end

err_sq_vec = mean(err_sq_targets, 2);
end
